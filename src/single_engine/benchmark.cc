...
auto BM_SingleEngineBlocking_Compress = [](benchmark::State &state,
  ...
  for (auto _ : state) {
    if (single_engine::compress(execution_path, qpl_default_level, source_buff,
                                mem_size, compressed_buff, &compressed_size)) {
...
auto BM_SingleEngineBlocking_SoftwareCompress_HardwareDecompress =
    [](benchmark::State &state, auto Inputs...) {
      ...
      zero_initialize_counters(state);
      TimeScope ts;
      if (single_engine::compress(compress_path, compression_level, source_buff,
                                  mem_size, compressed_buff,
                                  &compressed_size)) {
        LOG(FATAL) << "Failed to compress.";
        return;
      }
      ...
      size_t decompression_size = 0;
      for (auto _ : state) {
        if (single_engine::decompress(decompress_path, compressed_buff,
                                      compressed_size, decompressed_buff,
